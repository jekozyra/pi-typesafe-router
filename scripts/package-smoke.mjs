...
try {
  const output = JSON.parse(execFileSync("npm", ["pack", "--json", "--pack-destination", temp], { cwd: root, encoding: "utf8", stdio: ["ignore", "pipe", "inherit"] }));
  ...
  assert.ok(packed.files.every(file => /^(src\/[^/]+\.ts|examples\/[^/]+\.json|docs\/(privacy|configuration|architecture|releasing)\.md|README\.md|LICENSE|package\.json)$/.test(file.path)), "Unexpected package artifact");
  await writeFile(join(temp, "package.json"), '{"private":true,"type":"module"}\n');
  execFileSync("npm", ["install", "--ignore-scripts", "--no-audit", "--no-fund", join(temp, packed.filename), "@earendil-works/pi-coding-agent@0.85.1", "@earendil-works/pi-ai@0.85.1", "@earendil-works/pi-tui@0.85.1"], { cwd: temp, stdio: "inherit" });
...
`);
  execFileSync(process.execPath, ["smoke.mjs"], { cwd: temp, stdio: "inherit", env: {
    PATH: process.env.PATH, HOME: temp, TMPDIR: tmpdir(), PI_CODING_AGENT_DIR: join(temp, "profile"), PI_OFFLINE: "1", NO_COLOR: "1",
  } });
