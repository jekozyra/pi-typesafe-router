...
export type CredentialSource = { source: "env"; variable: string } | { source: "pi"; provider: string };
export type Backend =
  ...
  | { type: "vercel"; model: "typesafe-ai/jev"; auth: CredentialSource; zeroDataRetention: boolean };
...
export type ClassifierFailureCode = "credentials" | "timeout" | "cancelled" | "network" | "invalid-response" | "http";
export class ClassifierError extends Error {
  constructor(public readonly code: ClassifierFailureCode, public readonly status?: number) {
    super(status === undefined ? `Classifier ${code}` : `Classifier ${code} (HTTP ${status})`);
    this.name = "ClassifierError";
  }
}
export interface ClassifyOptions { signal: AbortSignal; apiKey: string }
export type Classify = (backend: Backend, state: ClassificationState, options: ClassifyOptions) => Promise<Classification>;
...
export interface CandidateCheck { target: Target; eligible: boolean; reason?: string }
