...
export function candidateChecks(
  ...
  return targets.map((target) => {
    ...
    const model = eligibility.models.find(
      (item) => item.provider === target.provider && item.id === target.model,
    );
    if (!model) return reject("unknown-model");
    if (
      !eligibility.available.some(
        (item) => item.provider === target.provider && item.id === target.model,
      )
    )
      return reject("unavailable");
    if (
      eligibility.scope.length &&
      !eligibility.scope.some(
    ...
    if (!positiveFinite(model.contextWindow) || !positiveFinite(model.maxTokens))
...
export function chooseRoute(
  ...
  if (
    ...
    confidence < 0 ||
    confidence > 1 ||
    !["quick", "standard", "deep"].includes(choice)
  )
    return config.uncertainRoute;
  return choice;
}
